...
class EditSubcat extends React.Component {
    constructor() {
        super()
        this.state = {
            allCategories: [],
    ...
    componentDidMount() {
        ...
        $.ajax({
            method: 'GET',
            url: `http://localhost:5000/subcategorie/${localStorage.getItem("subcat")}`,
    ...
    edit() {
        var data = {
            ...
            catId: this.state.allCategories.filter(cat => cat.category === this.state.category )[0].catId,
        ...
        $.ajax({
            ...
            url: `http://localhost:5000/subcategories/${localStorage.getItem('subcat')}`,
            contentType: "application/json",
            headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
            success: function (data) {
                console.log("dataaaa:", data)
            ...
            <div>
                ...
                <form style={{ marginTop: "8%", marginLeft: "30%", width: "40%" }} className="FORM">
                    <div className="form-group">
                        <input style={{ height: "35px" }} value ={this.state.subcat} type="text" className="form-control" placeholder="Subcategory Name" name="subcat" required="required" onChange={(e) => this.getTheInfo(e)} />
                    </div>
                    <br />

                    <select style={{ height: "35px" }} value = {this.state.category} name="category" className="form-control" required="required"
